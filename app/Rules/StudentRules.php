...
/**
 * Regras de validação reutilizáveis para Student.
 */
class StudentRules
{
    /**
    ...
    {
        $cpfUniqueRule = 'is_unique[students.cpf]';
        if ($ignoreId !== null) {
            $cpfUniqueRule = "is_unique[students.cpf,id,{$ignoreId}]";
        }

        return [
            'student.nome_completo' => 'required|min_length[3]|max_length[255]',
            'student.cpf'           => "required|exact_length[14]|{$cpfUniqueRule}",
